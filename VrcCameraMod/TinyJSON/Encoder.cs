	...
	public sealed class Encoder
	{
		...
		IEnumerable<FieldInfo> GetFieldsForType( Type type )
		{
			if (EnforceHierarchyOrderEnabled)
			{
				...
				while (types.Count > 0)
				{
					fields.AddRange( types[0].GetFields( BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance ) );
                    types.RemoveAt(0);
				}
		...
		IEnumerable<PropertyInfo> GetPropertiesForType( Type type )
		{
			if (EnforceHierarchyOrderEnabled)
			{
				...
				while (types.Count > 0)
				{
					properties.AddRange( types[0].GetProperties( BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance ) );
                    types.RemoveAt(0);
                }
